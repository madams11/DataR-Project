...
weather_related <- qplot(WEATHER, data = df4, geom = "bar",fill = I("darkblue"), alpha = I(0.8))
weather_related <- weather_related + ggtitle("Fatalities by Weather Type")
weather_related <- weater_related + scale_y_discrete(name = "Number of Fatalities")
weather_related <- weather_related + scale_x_discrete(name="Type of Weather")
weather_related
ggsave(filename = "Fatalities by Weather Type.jpg", plot = weather_related, width = 6, height = 4,
       units = "in")
...
dd_plot <- qplot(DRUNK_DR, data = df1, geom = "bar", fill = BIKECTYPE)
...
dd_plot <- dd_plot + scale_fill_brewer(palette = "Blues")
dd_plot <- dd_plot + theme(panel.grid.minor.x = element_blank())
dd_plot <- dd_plot + theme(panel.grid.major.x = element_blank())
dd_plot

ggsave(filename = "Crash Type By Driver.jpg", plot = dd_plot, width = 6, height = 4,
       units = "in")
